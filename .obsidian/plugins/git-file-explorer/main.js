...
__export(main_exports, {
  default: () => GitFileExplorerPlugin
});
module.exports = __toCommonJS(main_exports);
var import_obsidian6 = require("obsidian");

// src/fileExplorerHandler.ts
var import_obsidian = require("obsidian");
var FileExplorerHandler = class {
  constructor(app) {
    this.doWithFolderItem = async (func) => {
      var _a2, _b;
      return Promise.all(
        Object.values((_b = (_a2 = this.fileExplorer) == null ? void 0 : _a2.fileItems) != null ? _b : {}).filter(this.isFolder).map(func)
      );
    };
    this.isFolder = (item) => item.file instanceof import_obsidian.TFolder;
    this.fileExplorer = FileExplorerHandler.retrieveFileExplorer(app);
  }
  static retrieveFileExplorer(app) {
...
// src/widgets/utils/smartDebouncer.ts
var SmartDebouncer = class {
  ...
  debounce(key, fn) {
    if (key === "*") {
      this.timers.forEach((timer3) => clearTimeout(timer3));
      this.timers.clear();
      const timer2 = setTimeout(async () => {
        this.timers.delete("*");
        await fn();
      }, this.timeout);
      this.timers.set("*", timer2);
      return;
    }
...
// src/widgets/utils/eventBus.ts
var GitEventBus = class {
  constructor() {
...
var WidgetManager = class {
  constructor(factory, fileExplorerHandler, basePath) {
    this.factory = factory;
    this.fileExplorerHandler = fileExplorerHandler;
    this.basePath = basePath;
    this.widgets = [];
    this.smartDebouncer = new SmartDebouncer(3e3);
    ...
    this.updateExistingWidgets = async () => {
      await Promise.all(this.widgets.map((widget) => widget.update()));
    };
    this.addWidgetsForNewFolderItems = async () => await this.fileExplorerHandler.doWithFolderItem(async (folderItem) => {
      if (this.isNewFolderItem(folderItem))
        await this.createWidgetsForFolderItem(folderItem);
    });
    this.isNewFolderItem = (folderItem) => !this.widgets.some(
      (widget) => widget.getParent().isEqualNode(folderItem.selfEl)
    );
  ...
  async createWidgetsForFolderItem(folderItem) {
    try {
  ...
  getFullPathToItem(item) {
    return (0, import_path.join)(this.basePath, item.file.path);
  }
};
...
// src/git/utils/terminalExecutor.ts
...
var TerminalExecutor = class {
   ...
   */
  static async execute(workingDir, command) {
  ...
  static executeInWindowsTerminal(workingDir, command) {
    return new Promise((resolve, reject) => {
      const fullCommand = `start "" /B cmd /C "cd /d "${workingDir}" && ${command}"`;
      (0, import_child_process2.exec)(fullCommand, (error) => {
        if (error) {
  ...
  static executeInMacOSTerminal(workingDir, command) {
    return new Promise((resolve, reject) => {
      const appleCmd = `tell application "Terminal" to do script "cd '${workingDir}' && ${command}"`;
      (0, import_child_process2.exec)(`osascript -e '${appleCmd}'`, (error) => {
        if (error) {
  ...
  static executeInLinuxTerminal(workingDir, command) {
    return new Promise((resolve, reject) => {
      try {
        const tmpScriptPath = (0, import_path2.join)(os.tmpdir(), `terminal-cmd-${Date.now()}.sh`);
        const scriptContent = `#!/bin/bash
cd "${workingDir}" && ${command}
echo "Press Enter to close"
read
`;
        (0, import_fs.writeFileSync)(tmpScriptPath, scriptContent, { mode: 493 });
        const terminals = [
        ...
        for (const term of terminals) {
          try {
            const result = (0, import_child_process2.spawnSync)("which", [term.cmd]);
            if (result.status === 0) {
              const terminalCmd = `${term.cmd} ${term.args}`;
              (0, import_child_process2.exec)(terminalCmd, (error) => {
                try {
                  (0, import_fs.unlinkSync)(tmpScriptPath);
                } catch (e) {
                }
                if (error) {
        ...
        if (!found) {
          ...
          (0, import_child_process2.exec)(`cd "${workingDir}" && ${command}`, (error) => {
            if (error) {
...
var GitRepository = class {
  constructor(repoAbsPath) {
    this.repoAbsPath = repoAbsPath;
    this.remoteBranch = void 0;
    this.git = esm_default(this.repoAbsPath);
    this.eventBus = GitEventBus.getInstance();
  }
  notifyUpdate() {
  ...
  static async getInstance(repoAbsPath) {
  ...
  static isGitRepo(fullPath) {
    const gitDir = (0, import_path3.join)(fullPath, ".git");
    return (0, import_fs2.existsSync)(gitDir);
  }
  async getChangedFiles() {
    const status = await this.git.status();
    return status.files;
  }
  async getToPullCommitsCount() {
    if (!this.remoteBranch)
      return Promise.reject("No remote branch");
  ...
  async backup(commitMsg = "") {
    if (!commitMsg)
      commitMsg = "Backup @ " + new Date().toISOString();
    await this.stageAll();
    await this.commit(commitMsg);
  }
  static async openDiff(repoAbsPath, relativePath = "") {
    if (!GitRepository.isGitRepo(repoAbsPath))
      throw new Error("Not a git repository @ " + repoAbsPath);
    try {
      const gitCmd = relativePath ? `git difftool --no-prompt -- "${relativePath}"` : "git difftool --no-prompt";
...
// src/widgets/changesGitWidget.ts
var import_obsidian2 = require("obsidian");

// src/widgets/gitWidget.ts
var GitWidget = class {
  constructor(parent, gitRepository, widgetId) {
    this.parent = parent;
    ...
    this.updateText = (text) => this.widgetEl.textContent = text;
  ...
  install(widgetId) {
    let element = this.parent.querySelector(`#${widgetId}`);
    if (!element) {
  ...
  async executeWithSuccessAnimation(func) {
    this.disableEvents();
...
// src/widgets/navColorUpdater.ts
var path = __toESM(require("path"));
var NavColorUpdater = class {
  constructor(navFolderTitleEl, navColorStyle = "colored-text") {
    var _a2, _b, _c;
    this.navFolderEl = (_a2 = navFolderTitleEl.parentElement) != null ? _a2 : void 0;
    (_b = this.navFolderEl) == null ? void 0 : _b.addClass("git-widget-container");
    this.repoRelPath = (_c = navFolderTitleEl.getAttribute("data-path")) != null ? _c : void 0;
  ...
  update(gitNodes) {
    ...
    const cssRules = [];
    const fileStyle = this.navColorStyle === "colored-text" ? `color: #b38522 !important;` : `color: #b38522 !important; border-left: 3px solid var(--git-changed-file-color, #b38522);`;
...
// src/widgets/changesGitWidget.ts
var ChangesGitWidget = class extends GitWidget {
  constructor(navFolderTitleEl, gitRepository, app, promptCommitMsg = false, enableNavColorUpdater = true, navColorStyle = "colored-text") {
    super(navFolderTitleEl, gitRepository, "changes-git-widget");
    this.app = app;
    this.promptCommitMsg = promptCommitMsg;
    this.navColorStyle = navColorStyle;
    this.changesBuffer = 0;
    this.lastChangedPaths = /* @__PURE__ */ new Set();
    this.backupChanges = async (commitMsg = "") => this.executeWithSuccessAnimation(async () => {
  ...
  getName() {
    return "changes-git-widget";
  }
  async updateChanges() {
    var _a2;
    const changedNodes = await this.gitRepository.getChangedFiles();
    const newPaths = new Set(changedNodes.map((n) => n.path));
    if (this.setsEqual(newPaths, this.lastChangedPaths))
      return;
    this.lastChangedPaths = newPaths;
    (_a2 = this.navColorUpdater) == null ? void 0 : _a2.update(changedNodes);
  ...
  uninstall() {
    var _a2;
    super.uninstall();
    (_a2 = this.navColorUpdater) == null ? void 0 : _a2.cleanup();
  }
};
var CommitMsgModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    const textSetting = new import_obsidian2.Setting(contentEl).setName("Commit message").setDesc(
    ...
    new import_obsidian2.Setting(contentEl).addButton(
      (btn) => btn.setButtonText("Commit").setCta().onClick(this.submit.bind(this))
    );
  }
  submit() {
    this.close();
...
// src/widgets/utils/autoSyncManager.ts
var AutoSyncManager = class {
  constructor(syncFrequency = 0, syncOnStartup = false, syncCallback = null) {
...
// src/widgets/syncGitWidget.ts
var SyncGitWidget = class extends GitWidget {
  constructor(parent, gitRepository, autoSyncFrequency, autoSyncOnStartup) {
  ...
  getName() {
    return "sync-git-widget";
  }
  async updateSyncStatus() {
    try {
      const toPullCount = await this.gitRepository.getToPullCommitsCount();
      const toPushCount = await this.gitRepository.getToPushCommitsCount();
...
// src/widgets/gitWidgetFactory.ts
var GitWidgetFactory = class {
  constructor(app, settings) {
    this.app = app;
    this.settings = settings;
  }
  async buildWidgets(parent, repoAbsPath) {
    try {
  ...
  createChangesGitWidget(parent, repo) {
  ...
  createSyncGitWidget(parent, repo) {
...
var GitFileExplorerSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    let gitChangesSettingsContainer;
    new import_obsidian3.Setting(containerEl).setName("Activate git changes widget").setDesc(
      "Show a widget in the file explorer with a counter of the current staged and unstaged changes"
    ).setHeading().addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.gitChangesWidgetActive).onChange(async (value) => {
    ...
    let autoSyncSettingsContainer;
    new import_obsidian3.Setting(containerEl).setName("Activate git sync widget").setDesc(
      "Show a widget in the file explorer with a button to sync with the remote repository"
    ).setHeading().addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.gitSyncWidgetActive).onChange(async (value) => {
    ...
    containerEl.createEl("p").createEl("i").setText("Changes require restarting Obsidian");
  ...
  renderGitChangesSettings(container, isVisible) {
    container.empty();
    if (isVisible) {
      new import_obsidian3.Setting(container).setName("Enable prompting commit message").setDesc(
        "Prompt for a commit message when saving a file. If disabled, the commit message is in the format 'Backup @ {iso-timestamp}'"
      ).addToggle(
        (toggle) => toggle.setValue(this.plugin.settings.promptCommitMsg).onChange(async (value) => {
          this.plugin.settings.promptCommitMsg = value;
          await this.plugin.saveSettings();
        })
      );
      new import_obsidian3.Setting(container).setName("Change color of changed files in the file explorer").addToggle(
        (toggle) => toggle.setValue(this.plugin.settings.enableNavColorUpdater).onChange(async (value) => {
          this.plugin.settings.enableNavColorUpdater = value;
          await this.plugin.saveSettings();
        })
      );
      new import_obsidian3.Setting(container).setName("Changed files style").setDesc("Choose how changed files should be highlighted in the file explorer").addDropdown(
        (dropdown) => dropdown.addOption("colored-text", "Colored text").addOption("margin-highlight", "Margin highlight + colored text").setValue(this.plugin.settings.navColorStyle).onChange(async (value) => {
  ...
  renderAutoSyncSettings(container, isVisible) {
    container.empty();
    if (isVisible) {
      new import_obsidian3.Setting(container).setName("Auto-sync on startup").setDesc(
      ...
      new import_obsidian3.Setting(container).setName("Auto-sync frequency (minutes)").setDesc(
        "How often to automatically sync repositories (0 to disable periodic sync)"
      ).addSlider(
        (slider) => slider.setLimits(0, 240, 5).setValue(this.plugin.settings.autoSyncFrequency).setDynamicTooltip().onChange(async (value) => {
...
var _InitNewRepoHandler = class {
  constructor(basePath) {
    ...
    this.buildAbsPathTo = (path2) => (0, import_path4.join)(this.basePath, path2);
  }
  async execute(fileOrFolder) {
    if (fileOrFolder instanceof import_obsidian4.TFile) {
      const folder = fileOrFolder.parent;
      if (!folder)
        return;
      const absPath = this.buildAbsPathTo(folder.path);
      if (GitRepository.isGitRepo(absPath))
        return;
  ...
  getCommandName() {
    return _InitNewRepoHandler.INIT_NEW_REPO;
  }
  getIcon() {
    return "git-branch";
  }
  getCommandId() {
    return _InitNewRepoHandler.COMMAND_ID;
  }
};
var InitNewRepoHandler = _InitNewRepoHandler;
InitNewRepoHandler.INIT_NEW_REPO = "Init git repository";
InitNewRepoHandler.COMMAND_ID = "init-git-repo-active-folder";

// src/gitDiffHandler.ts
var import_obsidian5 = require("obsidian");
var import_path5 = require("path");
var _GitDiffHandler = class {
  constructor(basePath) {
    this.basePath = basePath;
    this.withCallback = (callback) => {
    ...
    this.buildAbsPathTo = (path2) => (0, import_path5.join)(this.basePath, path2);
  }
  async execute(fileOrFolder) {
    var _a2;
    const folderPath = fileOrFolder instanceof import_obsidian5.TFolder ? fileOrFolder.path : (_a2 = fileOrFolder.parent) == null ? void 0 : _a2.path;
    if (!folderPath)
      return;
  ...
  getCommandName() {
    return _GitDiffHandler.OPEN_GIT_DIFF;
  }
  getIcon() {
    return "git-pull-request";
  }
  getCommandId() {
    return _GitDiffHandler.COMMAND_ID;
  }
  // Find the git repository root by walking up the directory tree
  findGitRepoRoot(startPath) {
    let currentPath = startPath;
    while (currentPath && currentPath.length > 0) {
      if (GitRepository.isGitRepo(currentPath)) {
        return currentPath;
      }
      const parentPath = (0, import_path5.join)(currentPath, "..");
...
var GitDiffHandler = _GitDiffHandler;
GitDiffHandler.OPEN_GIT_DIFF = "Open git diff tool";
GitDiffHandler.COMMAND_ID = "open-git-diff-active-file";

// src/contextMenuInstaller.ts
var ContextMenuInstaller = class {
  constructor(plugin) {
    this.plugin = plugin;
  }
  installContextMenu(provider) {
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("file-menu", (menu, fileOrFolder) => {
        menu.addItem((item) => {
          item.setTitle(provider.getCommandName()).setIcon(provider.getIcon()).onClick(() => provider.execute(fileOrFolder));
        });
      })
    );
  }
...
var CommandRegister = class {
  constructor(plugin) {
    this.plugin = plugin;
  }
  registerCommandForActiveFile(provider) {
...
// main.ts
var GitFileExplorerPlugin = class extends import_obsidian6.Plugin {
  constructor() {
    super(...arguments);
    this.initialize = async () => {
      this.fileExplorerHandler = new FileExplorerHandler(this.app);
      if (!this.fileExplorerHandler.fileExplorer)
        return;
      ...
      const capabilityProviders = [
        new GitDiffHandler(this.getVaultBasePath()).withCallback(() => {
          var _a2;
          return (_a2 = this.widgetManager) == null ? void 0 : _a2.update();
        }),
        new InitNewRepoHandler(this.getVaultBasePath()).withCallback(() => {
          var _a2;
          return (_a2 = this.widgetManager) == null ? void 0 : _a2.update();
        })
      ];
  ...
  registerEventListeners(callback) {
    ...
    (_a2 = this.fileExplorerHandler.fileExplorer) == null ? void 0 : _a2.containerEl.addEventListener(
      "click",
      callback
    );
  }
  deregisterEventListeners(callback) {
    var _a2;
    (_a2 = this.fileExplorerHandler.fileExplorer) == null ? void 0 : _a2.containerEl.removeEventListener(
      "click",
      callback
    );
  }
